...
function IconButton({ children, onClick, active }) {
    return (
        <button
            onClick={onClick}
            className={`w-8 h-8 rounded flex items-center justify-center transition-colors ${active ? 'bg-[#43444b] text-white' : 'text-[#b5bac1] hover:bg-[#3b3d44] hover:text-[#dbdee1]'
                }`}
        >
            {children}
        </button>
